...
**Immediate next task:** Phase 4 (trust scores, promotion rules) — plus the manual items only a human with hardware can do — two-device E2E chat smoke test (needs the migrations applied + anonymous sign-in enabled), TalkBack/VoiceOver sweep (`docs/accessibility-audit.md`), TLS pin bundle, production tile provider (ADR-13).

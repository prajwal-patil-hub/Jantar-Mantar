...
- [x] **Road / route status** (ADR-31) — Drift **schema v4** `RouteReports`: a two-endpoint segment with a condition, a cause and a **mandatory** expiry, drawn as a hazard line under the pins. **No `open` condition exists** (a test asserts the enum stays that way) — crowd data cannot certify a road is safe; `cleared` retracts an earlier report and renders dotted. Reported by two taps, no GPS. **Local-only so far** and the UI says so — server table + RLS + verification path is the next step

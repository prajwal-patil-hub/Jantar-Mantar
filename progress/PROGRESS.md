...
- **Fixed by deleting the client write path, not narrowing it.** Creation seeds the creator via an AFTER INSERT trigger; joining goes through a SECURITY DEFINER `join_by_invite()` that validates, writes `pending` and spends the invite use atomically. A narrowed policy would still depend on enumerating every column an attacker must not set, and the next column added would reopen it.
- **The old suite missed it because it only tested reads** — "an outsider cannot read group X" — never the write that turns an outsider into an insider. 16 new assertions attack the membership write. Mutation-tested: restoring the old policy turns the roster, pins and invite-code assertions red, so the exploit is demonstrated rather than asserted.
- **The "map disappears when I zoom" bug was not in the map.** `web/index.html` had **no viewport meta tag at all**. Measured on an emulated iPhone 13 (390 css px): the app laid out at **980 css px**, mobile Safari's fallback — so every breakpoint tuned in ADR-34 was being evaluated against a viewport the device does not have, and without `user-scalable=no` a pinch is a *browser page-zoom* rather than a map gesture. Measured again after the fix: 390 px. Driving zoom programmatically through the whole range showed tiles rendering at every level, which is what ruled out flutter_map early.
- **The web build was phoning home twice.** CanvasKit came from `www.gstatic.com` at runtime, so the app would not start when Google is unreachable — the exact conditions this app exists for. Now `--no-web-resources-cdn`, with CI asserting the `useLocalCanvasKit` flag rather than grepping for the string (the CDN URL survives as the untaken branch, so the grep was a false positive on a correct build). Then the new CSP caught a second one nothing had found: **Roboto from `fonts.gstatic.com` on every load**, despite Noto Sans being bundled. Blocked, and verified the app still renders correctly.
- **Added a CSP.** GitHub Pages cannot set headers, so the meta tag is the only egress control the deployment has. `connect-src` pins egress to the tile host and this project's Supabase host. A Dart test reads `SupabaseConfig.url` and `MapConfig.urlTemplate` rather than a second copy, because a stale allowlist fails silently as "sync is just slow".
- **Demo Mode now announces itself on every screen.** It defaults ON and the hosted build is public, so every visitor saw fabricated camps, capacities, alerts and an admin queue with nothing on screen saying so. A screenshot of that is indistinguishable from real reporting. Persistent, non-dismissible, ranked above the offline strip: "this is not real" outranks "this may be old".
- **Added `supabase/probe_public_api.sh`** — attacks the deployed backend with the bundled key the way an attacker would. pgTAP proves the policies are *correct*; this proves they are *applied*, which is the claim that fails when a migration is written but never run. It aborts with exit 2 rather than reporting findings when the host is unreachable — a probe that reads a network error as "the server accepted my malicious write" would train you to ignore it.
- 252 Dart tests green (+17); 80 pgTAP assertions (+16); analyze + custom_lint clean; web release builds self-contained.

**Checked and found sound, so it does not get re-audited:** no URL-reachable routes at all (Navigator 1.0, no `go_router`, no `initialRoute`), so URL tampering cannot reach a screen the UI does not offer; `isAdminSession` reads server-set `app_metadata` and is display-gating only; the client never sends `state`, `verified_at` or `submitter_id`, and RLS rejects them if a crafted request does; no Dart source maps ship.

**Next:** the server half of route reports, then read-only CAP ingest.
---
